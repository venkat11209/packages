pipeline {
    ...
    stages {
        stage('BUILD START MAIL NOTIFICATION') {
            ...
            steps {
                script {
                    ...
                    // Send email notification when the build starts
                    ...
                    emailext (
                        ...
                        to: "$DEFAULT_MAILS,parvathi.g@fission.it," // Add cloud team and code push to git recipient's email address here
                        )
        ...
        stage('CLONING GIT') {
            steps {
                ...
                checkout(scm: [$class: 'GitSCM', 
                    ...
                    userRemoteConfigs: [[credentialsId: 'Github_sshkey', url: 'git@github.com:fissioninfotech/api.propflip.git']]])
        ...
        stage('DEPLOY') {
            steps {
			    ...
			    sh "rsync --rsh='ssh -p5456' -a ${WORKSPACE}/* root@103.170.86.77:/var/www/html/propflip-project/api.propflip.io"
		    	sh "rsync --rsh='ssh -p5456' -a ${WORKSPACE}/.env root@103.170.86.77:/var/www/html/propflip-project/api.propflip.io"
			    sh "rsync --rsh='ssh -p5456' -a ${WORKSPACE}/* root@103.170.86.82:/var/www/html/propflip-project/api.propflip.io"
		        sh "rsync --rsh='ssh -p5456' -a ${WORKSPACE}/.env root@103.170.86.82:/var/www/html/propflip-project/api.propflip.io"
        ...
        stage('PM2 RESTART') {
            steps {
			    ...
			    sh 'ssh -p5456 root@103.170.86.77 "pm2 restart 1"'
			    sh 'ssh -p5456 root@103.170.86.82 "pm2 restart 1"'
	...
	post {
        ...
        failure {
            ...
            attachLog: true,
		to: '$DEFAULT_RECIPIENTS,parvathi.g@fission.it,'
		      }
